...
#   game tree     patches/<stream>/*.patch              -> Lighthouse/
#   libultraship  patches-libultraship/<stream>/*.patch -> Lighthouse/libultraship/
...
if [ -d Lighthouse/.git/rebase-apply ]; then
    echo "Lighthouse/ has an interrupted 'git am' in progress." >&2
    echo "Finish or discard it first:  git -C Lighthouse am --abort" >&2
    exit 1
fi

PIN_SHA=$(sed -n 's/^PIN_SHA=//p' fetch.sh)
if [ "$(git -C Lighthouse rev-parse HEAD)" != "$PIN_SHA" ]; then
    echo "Lighthouse/ is not at the pristine pin ($PIN_SHA). Run ./fetch.sh first." >&2
    exit 1
fi
...
for stream in $(ordered_streams patches); do
    if ls "$stream"*.patch >/dev/null 2>&1; then
        ...
        git -C Lighthouse am --3way "$PWD/$stream"*.patch
    fi
done

# --- Lane 2: libultraship submodule — apply each stream (if the lane has any) ---
LUS_DIR="Lighthouse/libultraship"
if ls patches-libultraship/*/*.patch >/dev/null 2>&1; then
    LUS_PIN=$(git -C Lighthouse rev-parse "$PIN_SHA":libultraship)

...
usual Ghostship workflow still applies: Claude edits, **the maintainer (William Emerison Six <billsix@gmail.com>) builds + runs** (needed both to capture
...
2. **De-disassemble the ugly C** — **now owned by `tasks/archive/mario64/2026/09/23/mario64-assembly-isms-to-standard-c.md`**
...
(~379 `func_*` + ~431 `D_*`) are concentrated here — **confirm with the maintainer if they had specific files in
...
2. **Runtime logging when static analysis is inconclusive.** Add a *temporary* log at function entry
   ...
   toggleable — then **ask the maintainer to run the game and exercise the relevant scene** (the file-select
...
- **Behavior-preserving is the hard rule.** Renames must be total (def + all refs). Readability
  ...
  when unsure, leave it. Verify by the maintainer's build + in-game check, not just "it looks right."
...
- **Batch small and reviewable.** One file (or a cluster of related functions) per batch, so the maintainer can
...
with the maintainer, then the big ones (`renderer.c`, `joints.c`, `objects.c`), then `src/audio/`, then the
scattered `src/game/` functions.

## Progress log
- (empty — append one line per completed file/batch: what was renamed, any readability rewrites,
  build/run verified by the maintainer.)

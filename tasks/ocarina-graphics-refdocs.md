# OcarinaOfTime: graphics/math reference-doc set (umbrella)
...
Produce the same teaching-oriented graphics/math reference set for **Ship of Harkinian (Ocarina of Time)**
(OcarinaOfTime) that the mario64 initiative produced for Super Mario 64 — the topics
...
set exists under `tasks/reference/ocarina/` with a map/index.
...
them, do not re-derive.** This doc records only what is DIFFERENT for OcarinaOfTime.
...
- **Code under study:** `n64/OcarinaOfTime/Shipwright/` at pin `acdbc651d` (read the exact
  SHA + upstream from `n64/OcarinaOfTime/fetch.sh`; upstream `https://github.com/HarbourMasters/Shipwright`), three bodies of
  code — the game decomp, the port layer (`src/`), and **this game's own
  pinned libultraship submodule** at `62e973ae (1.3.1-486)`. Study THAT submodule.
- **Existing reference docs:** `tasks/reference/ocarina/` (per `n64/OcarinaOfTime/CLAUDE.md`) —
  architecture/orientation docs the new topic docs cross-link, never
  duplicate.

## What differs from mario64 (per-game)

- **Different decomp.** Ship of Harkinian (Ocarina of Time) has its own actor, scene, collision, animation,
...
- **Different libultraship pin** (`62e973ae (1.3.1-486)`). Pipeline/shader/texture/audio
...
1. **Seam-map survey** for this game's topics (same gap list, different
   locations).
2. **Foundation:** a `tasks/reference/ocarina/README.md` map + template, re-anchor stale
...
- This game's facts: `n64/OcarinaOfTime/CLAUDE.md`. Family contract: `n64/CLAUDE.md`.

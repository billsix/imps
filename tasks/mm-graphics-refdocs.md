# MajorasMask: graphics/math reference-doc set (umbrella)
...
Produce the same teaching-oriented graphics/math reference set for **2 Ship 2 Harkinian (Majora's Mask)**
(MajorasMask) that the mario64 initiative produced for Super Mario 64 — the topics
...
set exists under `tasks/reference/mm/ (create it)` with a map/index.
...
them, do not re-derive.** This doc records only what is DIFFERENT for MajorasMask.
...
- **Code under study:** `n64/MajorasMask/2ship2harkinian/` at pin `04a1a4319` (read the exact
  SHA + upstream from `n64/MajorasMask/fetch.sh`; upstream `https://github.com/HarbourMasters/2ship2harkinian`), three bodies of
  code — the game decomp, the port layer (`src/`), and **this game's own
  pinned libultraship submodule** at `7f2baa10 (1.3.1-397, pre-torch: ZAPDTR + OTRExporter)`. Study THAT submodule.
- **Existing reference docs:** `tasks/reference/mm/ (create it)` (per `n64/MajorasMask/CLAUDE.md`) —
  architecture/orientation docs the new topic docs cross-link, never
  duplicate.

## What differs from mario64 (per-game)

- **Different decomp.** 2 Ship 2 Harkinian (Majora's Mask) has its own actor, scene, collision, animation,
...
- **Different libultraship pin** (`7f2baa10 (1.3.1-397, pre-torch: ZAPDTR + OTRExporter)`). Pipeline/shader/texture/audio
...
1. **Seam-map survey** for this game's topics (same gap list, different
   locations).
2. **Foundation:** a `tasks/reference/mm/ (create it)README.md` map + template, re-anchor stale
...
- This game's facts: `n64/MajorasMask/CLAUDE.md`. Family contract: `n64/CLAUDE.md`.

...
#
# Patches are grouped into purpose STREAMS (subfolders): upstream-candidates,
# personal, etc. WITHIN a stream, order matters (numbered). ACROSS streams it
# usually does NOT — the streams touch disjoint files and therefore commute.
# Where one stream MUST precede the others, patches/ORDER pins the sequence
# (see n64/CLAUDE.md). See PaperMario/CLAUDE.md for what each patch does.
#
# Two lanes (n64/CLAUDE.md "Patches live in TWO lanes"), each streamed:
#   game tree     patches/<stream>/*.patch              -> PaperBoat/
#   libultraship  patches-libultraship/<stream>/*.patch -> PaperBoat/external/libultraship/
# (PaperBoat keeps libultraship under external/, unlike the other n64/ ports.)
...
if [ -d PaperBoat/.git/rebase-apply ]; then
    echo "PaperBoat/ has an interrupted 'git am' in progress." >&2
    echo "Finish or discard it first:  git -C PaperBoat am --abort" >&2
    exit 1
fi

PIN_SHA=$(sed -n 's/^PIN_SHA=//p' fetch.sh)
if [ "$(git -C PaperBoat rev-parse HEAD)" != "$PIN_SHA" ]; then
    echo "PaperBoat/ is not at the pristine pin ($PIN_SHA). Run ./fetch.sh first." >&2
    exit 1
fi
...
for stream in $(ordered_streams patches); do
    if ls "$stream"*.patch >/dev/null 2>&1; then
        ...
        git -C PaperBoat am --3way "$PWD/$stream"*.patch
    fi
done

# --- Lane 2: libultraship submodule — apply each stream (if the lane has any) ---
LUS_DIR="PaperBoat/external/libultraship"
if ls patches-libultraship/*/*.patch >/dev/null 2>&1; then
    LUS_PIN=$(git -C PaperBoat rev-parse "$PIN_SHA":external/libultraship)

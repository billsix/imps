...
| `patches/*.patch` | upstream code itself | the core case — covered by the pin-bump rebase procedure. PaperMario's LUS lane (`patches-libultraship/`) is keyed to the JeodC fork SHA in `.gitmodules`, not to the libultraship crawl's pin — replay it against the submodule's new SHA at a bump. |

...
# It refuses to run unless 2ship2harkinian/ is exactly at the pin, so it
# cannot double-apply the series or stack it onto the wrong base.
...
if [ "$(git -C 2ship2harkinian rev-parse HEAD)" != "$PIN_SHA" ]; then
    echo "2ship2harkinian/ is not at the pristine pin ($PIN_SHA)." >&2
    echo "Run ./fetch.sh first, then apply." >&2
    exit 1
fi

git -C 2ship2harkinian am --3way "$PWD"/patches/*.patch

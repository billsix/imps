#!/usr/bin/env bash
set -e
S=$S; : "${ASMDIFF_BUILD:?set ASMDIFF_BUILD to the configured Ghostship build tree}"; IMPS=$(git -C "$S" rev-parse --show-toplevel); G="$IMPS/n64/SuperMario64/Ghostship"; TOOL="$IMPS/tools"; cd "$G"

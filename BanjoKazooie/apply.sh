...
# It refuses to run unless Lighthouse/ is exactly at the pin, so it cannot
...
if [ "$(git -C Lighthouse rev-parse HEAD)" != "$PIN_SHA" ]; then
    echo "Lighthouse/ is not at the pristine pin ($PIN_SHA)." >&2
    echo "Run ./fetch.sh first, then apply." >&2
    exit 1
fi

git -C Lighthouse am --3way "$PWD"/patches/*.patch

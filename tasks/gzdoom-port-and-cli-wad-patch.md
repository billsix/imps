...
"Done" = `games/gzdoom/` builds GZDoom in a container from the pinned source, `tasks/reference/gzdoom/`
documents it, and a `patches/` change lets a CLI-passed WAD play without the GUI IWAD picker.
...
## Phase B — reference docs (`tasks/reference/gzdoom/`)

"Plenty of reference docs." Candidate topics (reuse the mario64 reference-set machinery — L0–L3 levels,
`file:line` anchors, per-topic docs — if wanted):
- **The IWAD vs PWAD model** and the resource/data system (`gzdoom.ini` `FileSearch`, the bundled `.pk3`s,
  load order) — the crux for the CLI-WAD goal.
- **The startup IWAD-selection GUI** — where it triggers (`d_iwad.cpp` / the IWAD picker), why it appears
  when the IWAD is ambiguous, and how the CLI flags relate — the code the Phase-C patch touches.
- **The CLI flags** (`-iwad`, `-file`, `-config`, `-norun`, …) and the launch/main flow.
- **The build system + dependency graph** (CMake, ZMusic, SDL2/audio/renderer), and the major subsystems
  (renderer, ZScript, sound via ZMusic) at whatever depth is useful.
...
## Open question (Phase C only)
1. **Patch shape.** The maintainer confirms the native CLI **did NOT reliably work** — passing a WAD via the
   flags failed, and he only got GZDoom to load one by dropping a WAD into a dot-folder under `~/` (a
   data-path/IWAD-search workaround). So a **real fix is needed** (not just a `run` wrapper), and it must
   make a **CLI-passed WAD actually play**. The exact shape — a new/clarified flag, single-positional-WAD
   auto-classify, or "skip the IWAD picker and honour the CLI WAD" — is decided in Phase C after reading the
   picker/data-path code (`d_iwad.cpp` et al.) and reproducing the failure with a real WAD + display.
